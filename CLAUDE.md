...
| Gate | Command | Covers | Baseline (measured 2026-07-31, Phase 5 Task 1.4 fix round) |
|---|---|---|---|
| Editor tests | `cd lib/editor && npx vitest run --no-file-parallelism` | `lib/editor`, `lib/theming`, plus shared `lib/*` modules it imports | **107 files / 1717 tests** — 1713 passed, **4 skipped**, ~68 s this run (Task 1.4's fix round added 6 tests to `video-track-remount.test.tsx`: the "wrap disagrees between sample and live call" proof, the `active`-independence-from-op-presence pin, and the wrap-on-edge-plate axis) (has read anywhere from 45-75s across the phase; the machine, not the suite, drives the variance). **A kind, task or warning added/removed moves this number — re-derive it per file, never carry a prior count forward.** |
...
| Pixel harness | `cd examples/layered-minimal && npm run pixel-gate:strict` — **while iterating, filter to the kinds you touched** (see below); this full form is for the gate itself | Every at-cut transition kind × mode × progress — one still per cell, hash-compared against committed goldens | **PASS** in **51 s** (300 stills, 0 retried): `300 accepted (9 on a bimodal cell's second recorded hash), 0 same-picture-different-bytes, 0 drifted, 0 missing`. Zero goldens moved — Task 1.4 touches `lib/render/` but no kind resolves to a `plan` node yet, so this is exactly the expected outcome, not a coincidence. `bimodalCells` is **24** (`clock-wipe` 9, `iris` 7, `light-leak` 8) — see the caveats immediately below before treating this as a plain pass/fail. |
