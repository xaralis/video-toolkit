...
|---|---|---|---|
| Editor tests | `cd lib/editor && npx vitest run --no-file-parallelism` | `lib/editor`, `lib/theming`, plus shared `lib/*` modules it imports | **109 files / 1809 tests** — 1803 passed, **6 skipped**, ~60 s this run (Task 4 migrated `checkerboard` — the catalog's LAST composite kind — to `plan`. Fix round 1 fixed two Criticals (group-opacity multiplying out the `'scale'`/`'flip'` ghosts; the mask id colliding across simultaneously-mounted boundaries sharing a cached node) and added an effective/ancestor-multiplied-opacity instrument. Fix round 2 (test-only, production code unchanged) added the OTHER half of that instrument — a real-tree assertion that the carve-out's `wrap` is inert (real mount visible) once its window closes, confirmed RED under a bare-`opacity:0` mutation first — corrected a comment that had gone stale the moment fix round 1 landed, and replaced a DOM-level test that false-passed against the pre-fix commit with one asserting the composite object directly. **A kind, task or warning added/removed moves this number — re-derive it per file, never carry a prior count forward.** |
...
| Pixel harness | `cd examples/layered-minimal && npm run pixel-gate:strict` — **while iterating, filter to the kinds you touched** (see below); this full form is for the gate itself | Every at-cut transition kind × mode × progress — one still per cell, hash-compared against committed goldens | **PASS** in **45 s** (300 stills, 0 retried): `300 accepted (7 on a bimodal cell's second recorded hash — varies run to run, per `bimodalCells`' own nature; re-derive rather than trust this figure), 0 same-picture-different-bytes, 0 drifted, 0 missing`. Task 4 moved `checkerboard` (the catalog's last composite kind) onto `plan` — its default `'fade'` (the only one of its 15 goldens' cells) via `LayerOp.wrap` (an SVG mask, mechanism UNCHANGED from Task 0.1's), `'scale'`/`'flip'` via `ghosts` (not part of the catalog default, so not golden-covered). **ZERO cells moved** — the filtered `checkerboard`-only run and the full unfiltered run both show 0 drifted, matching this task's own hypothesis (parts 1 and 2 are both specified pixel-exact), re-verified after fix round 1 (two Criticals fixed in `checkerboard.tsx`, neither touching the `'fade'` default's rendered pixels) and after fix round 2 (test-only, no production diff at all). `bimodalCells` unchanged at **18** (`clock-wipe` 9, `iris` 7, `light-leak` 2) — this task touched neither list nor any of those three kinds. |
