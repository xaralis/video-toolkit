...
| Gate | Command | Covers | Baseline (measured 2026-08-01, Phase 5 Task 2.2) |
|---|---|---|---|
| Editor tests | `cd lib/editor && npx vitest run --no-file-parallelism` | `lib/editor`, `lib/theming`, plus shared `lib/*` modules it imports | **108 files / 1747 tests** — 1742 passed, **5 skipped**, ~59 s this run (Task 2.2 added the reel-edge curtain regression test for `pixelate`, plus the `mountPlan` test-helper fix that lets ANY plan-arm test pass a genuinely null `LayerHandle`; `NODE_KINDS` grew from four to five with `gradient-wipe`'s migration, so the differential param-delivery skip count grew from 4 to 5 for the same structural reason). **A kind, task or warning added/removed moves this number — re-derive it per file, never carry a prior count forward.** |
