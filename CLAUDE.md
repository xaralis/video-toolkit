...
| Gate | Command | Covers | Baseline (measured 2026-07-31, Phase 5 Task 2.1) |
|---|---|---|---|
| Editor tests | `cd lib/editor && npx vitest run --no-file-parallelism` | `lib/editor`, `lib/theming`, plus shared `lib/*` modules it imports | **107 files / 1720 tests** — 1716 passed, **4 skipped**, ~63 s this run (Task 2.1 added 3 tests to `single-mount-assembly.test.tsx` proving a REAL catalog kind's progress reaches its `wrap` through the actual `buildVideoNodes` pipeline, no test registry; it also rewrote several existing tests/fixtures that used `fade`/`dissolve`/`clock-wipe`/`iris` as COMPOSITE-arm fixtures — those kinds moved to the `plan` arm this task, so the fixtures moved to `burn`, or to a new plan-aware mount helper, without changing test COUNT) (has read anywhere from 45-75s across the phase; the machine, not the suite, drives the variance). **A kind, task or warning added/removed moves this number — re-derive it per file, never carry a prior count forward.** |
...
| Pixel harness | `cd examples/layered-minimal && npm run pixel-gate:strict` — **while iterating, filter to the kinds you touched** (see below); this full form is for the gate itself | Every at-cut transition kind × mode × progress — one still per cell, hash-compared against committed goldens | **PASS** in **~50 s** (300 stills, 0 retried): `300 accepted (11-12 on a bimodal cell's second recorded hash — varies run to run, expected), 0 same-picture-different-bytes, 0 drifted, 0 missing`. **Zero goldens moved, confirmed across multiple separate processes (default `--repeat=2`, a filtered `--repeat=6` on just the six migrated kinds, a full `--repeat=4`)** — this is Task 2.1's FIRST PIXEL EVIDENCE from the `plan` path (`fade`, `dissolve`, `slide`, `flip`, `clock-wipe`, `iris`, colourless `fade-to-color` now resolve to it), not a regression check, and the zero movement is explained, not assumed: none of these six presentations ever set `mixBlendMode`, so the `isolate` flag flipping on for their own boundary has nothing to interact with in the harness's per-kind, single-transition compositions. `bimodalCells` is **24** (`clock-wipe` 9, `iris` 7, `light-leak` 8) — see the caveats immediately below before treating this as a plain pass/fail. |
