...
|---|---|---|---|
| Editor tests | `cd lib/editor && npx vitest run --no-file-parallelism` | `lib/editor`, `lib/theming`, plus shared `lib/*` modules it imports | **108 files / 1736 tests** — 1732 passed, **4 skipped**, ~63 s this run (Task 2.1 fix round 1 added a new file, `plan-neutral-progress.test.tsx` (15 tests), a DERIVED pin over `PLAN_KINDS` that the `wrap`'s neutral-outside-window progress is genuinely inert on both sides — the review's Important 2; plus 1 test to `transition-gallery.test.tsx` proving the gallery composition never hits `AtCutTransition`'s plan-arm-wrong-entry-point dead end for any `TRANSITIONS` entry — the review's Important 1) (has read anywhere from 45-75s across the phase; the machine, not the suite, drives the variance). **A kind, task or warning added/removed moves this number — re-derive it per file, never carry a prior count forward.** |
