...
### Gate rows record CRITERIA, never counts

**Do not write a test count, a file count, or a timing into this table. Do not "helpfully" put one
back.** Every gate below states what PASSING looks like — exit 0, zero failures, zero drifted, the
same three known type errors by identity — and that is the whole contract. Run the command, read
its output, act on it.

This is not tidiness, it is a scar. The editor-tests row carried a measured count for months and it
drifted **nine** times: it went stale on every branch that added a test, it was a merge conflict on
every concurrent branch (two sessions in one tree hit it in a single morning), and re-deriving it —
per commit, per file, with the arithmetic written out — cost real work in nearly every task. In
exchange it gated **nothing**: no run has ever failed because a count moved, and a count that cannot
fail a build but can be wrong is worse than no count at all. The same disease reached the
typecheck-coverage figures (a FLOOR, so upward drift passed silently and the recorded numbers went
stale unnoticed) and the pixel harness's tallies (which vary legitimately run to run).

Two counts survive, and only because the count IS the criterion: the brand-leak grep's **2** hits
and the `it.fails` guard's **zero**. Both fail the gate the moment they change, which is exactly
what the others never did.

Where a number genuinely matters, keep it where it can't lie: in an assertion (the skipped-test set
is asserted by the suite itself), in a script (the coverage guard prints its own counts), or in the
data (`bimodalCells` in the goldens file). Prose is the one place it will rot.

| Gate | Command | Covers | Passing looks like |
|---|---|---|---|
| Editor tests | `cd lib/editor && npx vitest run --no-file-parallelism` | `lib/editor`, `lib/theming`, plus shared `lib/*` modules it imports | **exit 0, 0 failed.** No counts here on purpose — see "Gate rows record criteria, never counts" below. The handful of SKIPPED tests are deliberate and the set that skips is itself asserted by the suite, so a kind cannot opt out quietly; read that assertion rather than a number written here. **Redirect, never pipe:** `> /tmp/gate.log 2>&1` — `cmd \| tee` reports tee's exit code, and this has already been read as a pass for a run that produced nothing. **Run the whole thing:** `vitest.config.ts` includes `{app,src,host}/**` PLUS `../{reel-config-base,theming,transcripts,render}/**`, so `vitest run app/ src/` is a FILTERED run — presenting one as the full gate has already happened once. |
...
| Render/transitions types | `cd examples/layered-minimal && npm run typecheck` | `lib/render` and `lib/transitions` (including their `.tsx` components), via the example that actually imports them — see `docs/superpowers/core-typecheck-gate.md` | **0 errors, coverage guard PASSES.** The guard prints a per-package file count and enforces a FLOOR, so counts drift upward silently and every figure ever written into this row went stale without a gate failing. Read the guard's own output; do not record it here. |
...
| Python — `sync_template` | `./.venv/bin/python -m pytest video_toolkit/tests/test_sync_template.py -q` | Template-scaffolding correctness; essentially never touched by render/transitions/editor work | **passes.** System `python3` has no `pytest` — use `./.venv/bin/python`. |
| Pixel harness | `cd examples/layered-minimal && npm run pixel-gate:strict` — **while iterating, filter to the kinds you touched** (see below); this full form is for the gate itself | Every at-cut transition kind × mode × progress — one still per cell, hash-compared against committed goldens | **PASS: `0 drifted, 0 missing`.** That is the whole criterion — the accepted/retried tallies vary legitimately between runs and are not a gate. The cells with two accepted hashes are recorded in the goldens file (`bimodalCells`); count them THERE when you need the number, never from prose, and read `docs/superpowers/transition-pixel-harness.md` for the mechanism. |
...
flip (9-50% depending on the cell) produces one of two *stable, recorded* attractor hashes on a
set of cells recorded as `bimodalCells` in the goldens file — **count them there when you need the
number** (a figure written into this prose contradicted the gate row and the paragraph below it for
three tasks running, which is why no count lives here any more). They are concentrated in the
rightmost 8 columns of the frame — this is
...
**The SKIPPED tests are deliberate and derived.** They are the generic "carries its authored params
through to the presentation" case for the kinds that resolve to a two-input node — a node closes
over its params, so there is no props bag to read. **The set of skipping kinds is itself asserted**,
so a kind cannot opt out quietly and the number needs no bookkeeping here: when `rgb-split` migrated
to a native node it joined that set automatically and the assertion is what noticed, not a
hand-edited count.

**What replaces them, precisely** (an earlier version of this paragraph claimed "pinned by
DOM assertions" and that was **false** for most of the params): the
...
progress points, and requires the rendered output to differ. Coverage is **derived from
`subOptionsFor`**, so a new param is covered the day it is added and no list here needs
maintaining. The one shape that needs its own case is an `accent`-typed param (`wipe.color`,
`fade-to-color.color`): there is no in-bounds probe value to invent, so it gets a dedicated
differential test instead of riding the generic loop. The block
fails whether the value is dropped at the forwarding table in
...
The harness records the known-**bimodal** cells (two accepted hashes each) in the goldens file's
`bimodalCells` — **read the list there**; it is the data, and it moves whenever a kind's picture
legitimately changes. Two things about re-seeding are worth knowing and are NOT visible in the
list:

- **Sample depth decides whether a second attractor can be recorded at all.** The script's
  `BIMODAL_RECORD_SAMPLES` is 8, so a `--repeat=6` re-seed cannot record one even where it exists —
  a shallow pass once "found" zero bimodal `light-leak` cells and a review caught it as
  structurally meaningless. `--repeat=24` is the depth that has actually recovered these.
- **A kind whose picture materially changes de-lists its old cells**, and that is correct rather
  than a silent drop: an old second hash does not describe a new picture. The harness prints
  `NOTE` lines when it happens.

The parenthesised "N matched a bimodal cell's SECOND recorded hash" in a gate run varies
legitimately between runs; `0 drifted, 0 missing` is the gate, that number is not.
