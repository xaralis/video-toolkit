...
| `checkerboard` | 64 clipped copies of `to` over an intact `from` | `from: {}`; `to.style.mask = url(#cells)`; one `over`-z-0 plate holding the SVG `<mask>` with `gridSize²` `<rect fill-opacity>`. |
...
| Task | What | Goldens |
|---|---|---|
| 0.1 | `checkerboard` as an SVG alpha mask over one mounted `to`. 66 → 3 media elements *inside today's model*. Pixel-exact for the default `'fade'`; the two other sub-options keep the cell path. | 15 cells (1 kind), likely unchanged |
