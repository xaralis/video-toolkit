...
|---|---|---|
| `lib/editor` (`cd lib/editor && npx tsc --noEmit`, baseline **3**) | `src`, `app`, `host`, `../theming`, plus **15** `lib/render` files and **16** `lib/transitions` files. Five of the render files are *declared* in that tsconfig's `include` — `at-cut-transitions.tsx`, `audio-track.tsx`, `layered-composition.tsx`, `video-track.tsx`, `load-fonts.ts` — so no test deletion can silently drop them; the other ten (`audio-gain.ts`, `transition-record.ts`, `video-track-layout.ts`, `video-track-plan.tsx`, `measure-sources.ts`, `fonts.ts`, `layered-composition-props.ts`, `overlay-anchor.ts`, `overlay-routing.ts`, `warn-once.ts`) and all of `lib/transitions` — **including `TransitionGallery.tsx`**, which arrives through `src/transition-gallery*.test.tsx` — arrive transitively (re-derived 2026-08-04 on `fix/brand-lane-derived` via `cd lib/editor && npx tsc --noEmit --listFiles \| grep 'lib/render/' \| grep -v node_modules`, redirected to a file rather than piped through a second stage that would swallow `tsc`'s own exit code: **15** hits, not the 14 this row previously recorded — `preview-environment.ts` is gone (deleted in Phase 5, its only consumer removed), and the row's prior 9-item transitive list had already dropped `video-track-plan.tsx`, which the count needs to stay accurate; `measure-sources.ts` also showed up in the listing and was never accounted for in either count, so it's added here rather than silently reproducing the same gap) | anything no editor file or declared entry point reaches |
| `examples/layered-minimal` (this gate, baseline **0**) | its own `src`, plus every `lib/**` file the render actually pulls in — `lib/render` (**13** files, including the 4 `.tsx` components above), `lib/transitions` (**16** files — every presentation, `index.ts`, `edge-plate.tsx`, and `TransitionGallery.tsx`; see below), `lib/theming` (**26**), `lib/reel-config-base` (**14**), `lib/transcripts` (**1**) | anything no reel imports (editor UI, host) |
...
**They went stale AGAIN, and here is the mechanism that guarantees they will keep doing so.** Re-measured 2026-08-04 on `fix/brand-lane-derived`: the guard prints **13 / 16 / 26 / 14 / 1**, where this file had recorded 12 / … / 10 (the row above is now the measured set). Only `reel-config-base` +1 belonged to that branch (`content-end.ts`); the rest was earlier work. **`EXPECTED_MINIMUMS` in `verify-typecheck-coverage.mjs` is a set of FLOORS (`9 / 14 / 23 / 8 / 1`), so a count drifting UPWARD passes silently** — nothing fails, nothing prints a warning, and the documented numbers rot while the gate stays green. The floors are the right choice for the gate's actual job (catching a *collapse* in coverage, e.g. a deleted test dropping a whole transitive subtree), so this is a documentation-freshness problem, not a guard bug. The consequence for a reader: **the only trustworthy source for these five numbers is a fresh `npm run typecheck` run** — not this file, not `CLAUDE.md`, and not the same numbers quoted back at you in a task brief.
