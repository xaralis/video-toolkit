...
Move both the source files (Root.tsx, SCREENPLAY.md, project.json, transcripts, brand assets) **and** the heavy media (raw footage, music, renders) between the local project, the toolkit git remote, and Cloudflare R2 — in one command. Non-technical collaborators must never run raw `git` commands; `/sync` handles them.

## Quick start

```
/sync                            # interactive: ask push or pull + scope
/sync push                       # commit projects/<name>/, push to git, then upload media to R2
/sync pull                       # git pull --rebase, then download media from R2
/sync list                       # git pull --rebase, then show R2 inventory
/sync share                      # git pull --rebase, then generate presigned public URL
/sync push out                   # just the renders (after /render) — still does git step
/sync push recordings,broll      # just raw footage — still does git step
/sync push --dry-run             # preview both git diff and R2 transfer
/sync <project-name> ...         # explicit project
```
...
If missing: stop and tell the user to set `R2_BUCKET_NAME`, `R2_ACCOUNT_ID`, `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` in `.env` (or run `/setup`).
...
if [ -n "$(git status --porcelain)" ]; then
  STASH_MSG="auto-stash by /sync at $(date -u +%FT%TZ)"
...
- **Rebase conflict** (`PULL_RC != 0`): abort the entire `/sync` operation. Do not proceed to R2. Report:
...
1. **Toolkit commit** — shared code/assets the project's render depends on
   ...
   for anyone who pulls unless that `lib/` code ships too — so `/sync push`
...
- **Just pulled?** → "Footage in place. Run `/cut` to (re)map clips, or `/fine-tune` to iterate in Studio."
- **Pushed recordings only?** → "Footage backed up. Continue with `/cut`."
- **Pushed out/ only?** → "Render in R2. Use `/sync share` to get a public URL."
- **Pushed everything?** → "Project mirrored. Safe to clean the local copy if disk pressure."

## Workflow integration

`/sync` is a regular part of the campaign-reels workflow — not optional. The per-project flow:

```
/video                                    # create / resume project
/narrate                                  # author SCREENPLAY.md
(film footage; drop into public/recordings + public/broll)
/sync push recordings,broll               # ← code + raw footage to git+R2
/cut                                      # map footage → defaultProps
/fine-tune                                # iterate in Studio
/add-music                                # generate bg.mp3 (optional)
/render                                   # produce reel.mp4
/sync push out                            # ← code + renders to git+R2
```

For collaborators joining mid-project:

```
/video                                    # detects existing project (resume)
/sync pull                                # ← code + all media in one step
/fine-tune  (or wherever the work is)
```
...
- **New projects:** `/sync list <name>` returns empty R2 inventory until the first push — that's expected.

## Re-run semantics

Running `/sync push` twice in a row is safe: the second run has no source changes (git step skips commit+push) and R2 skips size-matched files. Same for `pull`. Use `--overwrite` to force R2 re-transfer.
...
| "R2 not configured" | Set `R2_*` env vars in `.env` or run `/setup`. |
